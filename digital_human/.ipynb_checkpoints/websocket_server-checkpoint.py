        ...
        if self.avatar_manager:
            ...
            for avatar_id, avatar in self.avatar_manager.get_all_avatars().items():
                ...
                avatars.append({
                    ...
                    "has_idle_video": bool(getattr(avatar, "idle_video", None) and _os.path.exists(avatar.idle_video)),
                })
            ...
            if current:
                ...
                await self.connection_manager.send_message(
                    ...
                    WSMessage(MessageType.AVATAR_SWITCHED.value, {
                        ...
                        "has_idle_video": bool(getattr(current, "idle_video", None) and _os.path.exists(current.idle_video)),
                    })
                )
    ...
    async def _process_text_request(
        ...
        try:
            ...
            if not video_path or not os.path.exists(video_path):
                raise Exception(f"视频文件生成失败: {video_path}")

            video_size = os.path.getsize(video_path)
            if video_size < 1000:  # 视频文件太小，可能有问题
                raise Exception(f"视频文件异常: 大小仅 {video_size} bytes")

            print(f"[WS] 视频生成完成: {video_path}, 大小: {video_size} bytes")
