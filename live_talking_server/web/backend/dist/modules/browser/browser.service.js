...
exports.browserService = exports.PlaywrightBrowserService = void 0;
...
const HIGH_SPEED_STATION_SUFFIXES = ["北", "东", "南", "西", "龙门"];
const HIGH_SPEED_STATION_OVERRIDES = {
    资阳: ["资阳北"],
    洛阳: ["洛阳龙门"]
};
...
function buildStationSearchTargets(city, preferHighSpeedStation) {
    const normalizedCity = normalizeText(city);
    if (!normalizedCity) {
        return [];
    }
    const targets = new Set([normalizedCity]);
...
async function emitBrowserProgress(reporter, level, message, payload = {}) {
...
function extractLeadingChineseName(value) {
    const matched = normalizeText(value).match(/^[\u3400-\u9FFF路]+/u);
    return matched?.[0] ?? "";
}
function scoreCityOption(optionText, targetCity) {
...
function inferTrainType(trainNo) {
...
function normalizeSeatStatus(rawValue) {
...
function toSeatType(value) {
    return value;
}
function buildSeatInventory(seatType, rawValue) {
    return {
...
function calculateCandidateScore(candidate, index) {
    ...
    if (secondClassStatus === "有") {
        score += 16;
    }
    else if (secondClassStatus === "数字") {
        score += 12;
    }
    else if (secondClassStatus === "候补") {
        score += 8;
    }
    if (firstClassStatus === "有") {
        score += 8;
    }
    else if (firstClassStatus === "数字") {
        score += 5;
    }
    else if (firstClassStatus === "候补") {
        score += 3;
    }
...
async function closeBrowserPage(session) {
    await session.context.close();
    await session.browser.close();
}
async function gotoLeftTicketPage(page) {
    try {
...
async function waitForVisibleSelector(page, selectors, timeoutMs = 5000) {
...
async function clearStationInput(page, locator) {
    await locator.click({
        timeout: 5000
    });
...
async function clickCitySuggestion(page, targetCities) {
...
async function applyStationFallback(page, inputSelector, hiddenCodeSelector, targetCities) {
    ...
    await page.evaluate(({ nextCity, nextCode, visibleSelector, hiddenSelector }) => {
        const doc = globalThis.document;
        const visibleInput = doc?.querySelector(visibleSelector);
        const hiddenInput = doc?.querySelector(hiddenSelector);
...
async function fillStationField(page, inputSelector, hiddenCodeSelector, city, preferHighSpeedStation = false) {
    if (!city) {
        return false;
    }
    ...
    let hiddenCode = normalizeText(await page.locator(hiddenCodeSelector).first().inputValue().catch(() => null));
    if ((!selected || !hiddenCode || !visibleValue) && city) {
        const fallbackApplied = await applyStationFallback(page, inputSelector, hiddenCodeSelector, stationTargets);
        if (fallbackApplied) {
            await page.waitForTimeout(200);
...
async function waitForVisibleResultRows(page, timeoutMs = RESULT_WAIT_TIMEOUT_MS) {
...
async function getHeaderTexts(page) {
...
async function safeTextContent(locator, timeoutMs = 800) {
...
async function safeAttribute(locator, name, timeoutMs = 800) {
    const count = await locator.count().catch(() => 0);
    if (count === 0) {
        return null;
    }
...
function rankParsedRowsByIntent(parsedRows, intent) {
    ...
    const decorated = parsedRows.map((row, index) => {
        ...
        const trainTypePenalty = intent.trainTypes.length > 0 && !intent.trainTypes.includes(inferTrainType(row.trainNo)) ? 1 : 0;
        return {
            row,
            index,
            priorityBucket,
...
async function parseVisibleTrainRows(page, intent) {
    ...
    for (let index = 0; index < rowCount; index += 1) {
        ...
        const originStation = normalizeText(await safeAttribute(infoCell.locator(".start-s"), "title")) ||
            normalizeText(await safeTextContent(infoCell.locator(".start-s")));
        const destinationStation = normalizeText(await safeAttribute(infoCell.locator(".end-s"), "title")) ||
        ...
        const arrivalTime = normalizeText(await safeTextContent(infoCell.locator(".cds strong").nth(1))) ||
        ...
        const durationText = normalizeText(await safeTextContent(infoCell.locator(".ls strong"))) ||
            normalizeText(await safeTextContent(infoCell.locator(".ls")));
        const cellTexts = await extractRowCellTexts(row);
        const firstClassStatus = firstClassCellIndex !== null && firstClassCellIndex < cellTexts.length
            ? cellTexts[firstClassCellIndex]
            : "--";
        const secondClassStatus = secondClassCellIndex !== null && secondClassCellIndex < cellTexts.length
...
async function findVisibleTrainRow(page, candidateTrainNo) {
    const rows = page.locator(RESULT_ROW_SELECTOR);
    const rowCount = await rows.count();
    for (let index = 0; index < rowCount; index += 1) {
        const row = rows.nth(index);
...
async function clickPreorderButton(row) {
...
async function waitForLoginInterception(page, timeoutMs = 15000) {
    const urlPromise = page
        ...
        .catch(() => null);
    const selectorPromises = LOGIN_MODAL_SELECTORS.map((selector) => page
        .locator(selector)
        .first()
        .waitFor({
        state: "visible",
...
async function waitForAnyVisibleSelector(page, selectors, timeoutMs) {
    const selectorPromises = selectors.map((selector) => page
        .locator(selector)
        .first()
        .waitFor({
        state: "visible",
...
async function getCurrentlyVisibleSelector(page, selectors) {
    for (const selector of selectors) {
...
async function ensureTrainRowVisible(page, intent, candidateTrainNo, reporter) {
...
async function waitForPassengerPageReady(page, timeoutMs) {
    ...
    const urlPromise = page
        ...
        .catch(() => null);
    const selectorPromise = waitForAnyVisibleSelector(page, PASSENGER_READY_SELECTORS, timeoutMs).then((selector) => selector
        ? {
            mode: `visible_selector:${selector}`,
            currentUrl: page.url()
        }
...
async function clickFirstVisibleSelector(page, selectors, timeoutMs = 10000) {
    const startedAt = Date.now();
    while (Date.now() - startedAt < timeoutMs) {
        for (const selector of selectors) {
...
async function extractCheckboxText(checkbox) {
    return normalizeText(await checkbox
        .evaluate((node) => {
        const input = node;
        return (input.closest?.("li,label,tr,div")?.textContent ??
            input.parentElement?.textContent ??
            "");
    })
        .catch(() => null));
}
async function ensureCheckboxSelected(checkbox) {
...
async function waitForPassengerListReady(page, timeoutMs = 15000) {
...
async function collectVisiblePassengerOptions(page) {
    const passengerOptions = [];
    const preferredContainers = page.locator("#normal_passenger_id li, #normal_passenger_id label, ul#normal_passenger_id li, ul#normal_passenger_id label");
...
async function trySelectPassengerByName(page, passengerName) {
...
async function selectPassengers(page, passengerNames) {
...
function resolveSeatAliases(seatPreferences) {
...
async function selectSeatPreference(page, seatPreferences) {
    ...
    if (await selectLocator.isVisible().catch(() => false)) {
        ...
        const matchedOption = options.find((option) => aliases.some((alias) => normalizeCompactText(option.text).includes(alias)));
...
async function waitForPaymentPageReady(page, timeoutMs) {
    if (PAYMENT_PAGE_URL_PATTERN.test(page.url()) ||
    ...
    const urlPromise = page
        ...
        .catch(() => null);
    const selectorPromise = waitForAnyVisibleSelector(page, PAYMENT_READY_SELECTORS, timeoutMs).then((selector) => selector
        ? {
            mode: `visible_selector:${selector}`,
            currentUrl: page.url()
        }
...
async function waitForSubmitOutcome(page, timeoutMs) {
    const confirmPromises = FINAL_CONFIRM_SELECTORS.map((selector) => page
        .locator(selector)
        .first()
        .waitFor({
        state: "visible",
        ...
        .catch(() => null));
    const paymentPromise = waitForPaymentPageReady(page, timeoutMs).then((result) => result
        ? {
            kind: "payment_ready",
            mode: result.mode,
...
function mapToTrainCandidates(parsedRows) {
...
async function runRealTrainSearch(page, intent, reporter) {
    ...
    const originFilled = await fillStationField(page, FROM_STATION_INPUT_SELECTOR, FROM_STATION_CODE_SELECTOR, intent.origin, preferHighSpeedStation);
    if (!originFilled) {
    ...
    const destinationFilled = await fillStationField(page, TO_STATION_INPUT_SELECTOR, TO_STATION_CODE_SELECTOR, intent.destination, preferHighSpeedStation);
    ...
    await emitBrowserProgress(reporter, "DATA", `[读取结果] 真实车次 ${parsedRows.length} 条：${parsedRows.map((row) => row.trainNo).join(", ")}`, {
        count: parsedRows.length,
        trainNos: parsedRows.map((row) => row.trainNo)
    });
    return mapToTrainCandidates(parsedRows);
}
async function openLeftTicketPage(waitMs) {
...
class PlaywrightBrowserService {
    ...
    async execute(request) {
        switch (request.action) {
            case task_enums_1.BrowserActionType.SEARCH_TRAINS:
            ...
            case task_enums_1.BrowserActionType.WAIT_LOGIN:
                return this.waitLogin(request.taskId, request.payload.timeoutMs, request.onProgress);
            case task_enums_1.BrowserActionType.SELECT_TRAIN:
                return this.selectTrain(request.taskId, request.payload.intent, request.payload.candidateTrainNo, request.payload.seatType, request.payload.passengerNames, request.onProgress);
            case task_enums_1.BrowserActionType.SUBMIT_ORDER:
                return this.submitOrder(request.taskId, request.onProgress);
            default:
                return {
                    ok: false,
                    state: "failed",
                    errorCode: error_codes_1.ErrorCode.INTERNAL_ERROR,
                    errorMessage: `Unsupported browser action: ${request.action}`
                };
        }
    }
    async searchTrains(intent, onProgress) {
        const session = await createBrowserPage();
        let navigationSucceeded = false;
        try {
    ...
    async waitLogin(taskId, timeoutMs, onProgress) {
        ...
        try {
            ...
            if (!passengerReady) {
                const stillVisibleLoginSelector = await getCurrentlyVisibleSelector(taskSession.page, LOGIN_MODAL_SELECTORS);
                if (stillVisibleLoginSelector) {
                    this.humanVerifiedTasks.delete(taskId);
                    await emitBrowserProgress(onProgress, "WARN", `[登录拦截] 登录界面仍然可见：${stillVisibleLoginSelector}`, {
                ...
                const targetRow = await ensureTrainRowVisible(taskSession.page, taskSession.intent, taskSession.candidateTrainNo, onProgress);
            ...
            await emitBrowserProgress(onProgress, "HIT", `[乘客匹配] ${passengerSelection.selectedPassengers.join(", ") || "默认首位乘客"}`, {
            ...
            await emitBrowserProgress(onProgress, "DATA", `[席别匹配] ${seatSelection.requestedSeat ?? "默认席别"} -> ${seatSelection.matchedOption ?? "保持页面默认"}`, {
    ...
    async selectTrain(taskId, intent, candidateTrainNo, seatType, passengerNames, onProgress) {
    ...
    async submitOrder(taskId, onProgress) {
        const taskSession = this.getTaskSession(taskId);
        if (!taskSession) {
            return {
                ok: false,
...
exports.browserService = new PlaywrightBrowserService();
