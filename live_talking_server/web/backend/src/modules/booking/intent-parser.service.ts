...
export interface IntentParserService {
  parse(
    rawText: string,
    previousIntent?: Partial<BookingIntent>
  ): Promise<IntentParserResult>;
}

const DEFAULT_SEAT_TYPE = "二等座" as SeatType;

const SEAT_TYPE_RULES: Array<{ seatType: SeatType; keywords: string[] }> = [
  { seatType: "商务座" as SeatType, keywords: ["商务座", "商务"] },
  { seatType: "特等座" as SeatType, keywords: ["特等座", "特等"] },
  { seatType: "一等座" as SeatType, keywords: ["一等座", "一等"] },
  { seatType: "二等座" as SeatType, keywords: ["二等座", "二等"] },
  { seatType: "高级软卧" as SeatType, keywords: ["高级软卧"] },
  { seatType: "软卧" as SeatType, keywords: ["软卧", "动卧"] },
  { seatType: "硬卧" as SeatType, keywords: ["硬卧"] },
  { seatType: "软座" as SeatType, keywords: ["软座"] },
  { seatType: "硬座" as SeatType, keywords: ["硬座"] },
  { seatType: "无座" as SeatType, keywords: ["无座"] }
];

const RELATIVE_DAY_OFFSETS: Array<{ keyword: string; days: number }> = [
  { keyword: "大后天", days: 3 },
  { keyword: "后天", days: 2 },
  { keyword: "明天", days: 1 },
  { keyword: "今天", days: 0 },
...
function resolveMonthDayDate(month: number, day: number, now: Date): string | null {
...
function resolveWeekdayDate(prefix: string, weekday: string, now: Date): string | null {
...
function extractTravelDate(rawText: string, now = new Date()): string | null {
  const normalizedText = rawText.replace(/\s+/g, "");

  const fullDateMatch =
    normalizedText.match(/(\d{4})[-/.年](\d{1,2})[-/.月](\d{1,2})日?/) ??
  ...
  const monthDayMatch =
    normalizedText.match(/(\d{1,2})月(\d{1,2})[日号]?/) ??
...
function normalizeLocation(value: string | undefined): string | null {
  ...
  return value
    .replace(
      /(?:的)?(?:商务座|特等座|一等座|二等座|高级软卧|软卧|硬卧|软座|硬座|无座|高铁|动车|火车|列车|车票|票|班次|出发|发车).*/u,
      ""
    )
    .replace(/[，。,.；;、\s].*$/, "")
    .trim() || null;
...
function extractRoute(rawText: string): { origin: string | null; destination: string | null } {
  const routePatterns = [
...
function extractSeatPreferences(rawText: string): SeatType[] {
  const matchedSeatTypes: SeatType[] = [];
...
function extractTrainTypes(rawText: string): TrainTypeCode[] {
  const trainTypes = new Set<TrainTypeCode>();
...
function applyDayPeriod(hours: number, period: string | null): number {
  if (!period) {
    return hours;
  }
...
function extractExactDepartureTime(rawText: string): string | null {
  ...
  const chineseClockMatch = rawText.match(
    /(凌晨|早上|上午|中午|下午|傍晚|晚上|今晚|夜里|夜间)?\s*(\d{1,2})\s*(?:点|时)(半|一刻|三刻|(\d{1,2})分?)?/
  );
...
function extractDepartureWindow(rawText: string): {
  departureTimeLowerBound: string | null;
  departureTimeUpperBound: string | null;
} {
...
function collectMissingSlots(intent: BookingIntent): BookingSlotName[] {
  const missingSlots: BookingSlotName[] = [];
...
export function mockParseIntent(rawText: string, taskId: ID): IntentParserResult {
