...
def build_nerfreal(sessionid:int)->BaseReal:
    ...
    elif opt.model == 'fasterliveportrait':
...
async def list_avatars(request):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return web.Response(
                content_type="application/json",
                text=json.dumps({"code": -1, "msg": "Avatar config not found"}),
            )
        ...
        for avatar_id, info in config.get("avatars", {}).items():
            ...
            # 添加待机视频URL
            if info.get("idle_video"):
                avatar_data["idle_video"] = "/human_choice/" + info["idle_video"]
        ...
        return web.Response(
            ...
            text=json.dumps({
                ...
                "data": {
                    "avatars": avatars,
                    "default": config.get("default_avatar", avatars[0]["id"] if avatars else "")
                }
            }),
        )
    except Exception as e:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='musetalk') #musetalk wav2lip ultralight fasterliveportrait
    ...
    if opt.model == 'musetalk':
        ...
        from fasterlivereal import FasterLivePortraitReal,load_model,load_avatar,warm_up
        logger.info(opt)
        model = load_model()
        avatar = load_avatar(opt.avatar_id, model[0])  # model is (pipeline, joyvasa)
    ...
    # 添加 Human_Choice 目录作为静态文件服务（用于待机视频）
    human_choice_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Human_Choice")
