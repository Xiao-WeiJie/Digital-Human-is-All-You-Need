...
let ws = null;
let idleVideoUrl = null;  // 当前 avatar 的待机视频 URL

// ============================================================
// 待机视频管理
// ============================================================

async function loadIdleVideo(avatarId, hasIdleVideo) {
    if (!hasIdleVideo) {
        idleVideoUrl = null;
        return;
    }
    const apiUrl = elements.apiUrlInput?.value || CONFIG.apiUrl;
    idleVideoUrl = `${apiUrl}/idle/${avatarId}`;
    playIdleVideo();
}

function playIdleVideo() {
    if (!idleVideoUrl) return;
    console.log('[Idle] 切换到待机视频:', idleVideoUrl);
    elements.avatarVideo.loop = true;
    elements.avatarVideo.muted = true;
    elements.avatarVideo.src = idleVideoUrl;
    elements.avatarVideo.load();
    elements.avatarVideo.play().catch(e => {
        console.log('[Idle] 待机视频自动播放被阻止', e);
    });
...
function handleAvatarSwitched(data) {
    ...
    // 加载并循环播放待机视频（has_idle_video 由服务端直接下发）
    loadIdleVideo(data.avatar_id, data.has_idle_video ?? false);
}
...
function handleVideoReady(data) {
    ...
    if (data.video_url) {
        ...
        // 使用 API 获取视频
        const apiUrl = elements.apiUrlInput?.value || CONFIG.apiUrl;
        // 确保 video_path 正确编码，但不编码开头的斜杠
        let videoPath = data.video_path;
        // 如果是绝对路径（以 / 开头），保持原样
        // encodeURIComponent 会把 / 编码成 %2F，我们需要保留它
        const videoUrl = `${apiUrl}/video/${videoPath}`;
...
function playVideo(url) {
    ...
    elements.avatarVideo.loop = false;   // 回复视频不循环
    elements.avatarVideo.muted = false;  // 确保取消静音
    elements.avatarVideo.src = url;
    elements.avatarVideo.load();  // 重新加载视频

    // 视频加载完成事件
    elements.avatarVideo.onloadedmetadata = () => {
        console.log('[Video] 视频元数据加载完成，时长:', elements.avatarVideo.duration);
    };

    elements.avatarVideo.onloadeddata = () => {
        console.log('[Video] 视频数据加载完成，开始播放');
        elements.avatarVideo.play().catch(e => {
    ...
    elements.avatarVideo.onerror = (e) => {
        ...
        console.error('[Video] 错误代码:', elements.avatarVideo.error?.code);
        console.error('[Video] 错误信息:', elements.avatarVideo.error?.message);
    };

    // 视频播放结束事件
    elements.avatarVideo.onended = () => {
    ...
    // 视频卡住事件
    elements.avatarVideo.onstalled = () => {
        console.warn('[Video] 视频加载停滞');
    };

    elements.avatarVideo.onwaiting = () => {
        console.log('[Video] 视频缓冲中...');
    };

    elements.avatarVideo.oncanplay = () => {
